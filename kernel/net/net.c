...
static bool net_send_arp(netif_t *netif, const uint8_t srcmac[6], const uint8_t dstmac[6], enum arp_opcode opcode, const uint8_t srchw[6], const uint8_t srcpr[6], const uint8_t dsthw[6], const uint8_t dstpr[4]) {
	...
	bool success = net_send_ethernet(netif, srcmac, dstmac, ETHERNET_TYPE_ARP, (uint8_t *)packet, size);
	kfree(packet);
	return success;
}
...
//	packet->flags_fragment_offset = htons(0x4000);
	...
	bool success = false;
	if (dstmac != NULL) {
		success = net_send_ethernet(netif, netif->mac, dstmac, ETHERNET_TYPE_IPV4, (uint8_t *)packet, size);
	} else {
...
static void net_handle_icmp_echo(netif_t *netif, const ethernet_packet_t *ethernet_packet, const ipv4_packet_t *ipv4_packet, const icmp_packet_t *icmp_packet, size_t icmp_packet_data_size) {
	if (icmp_packet_data_size < sizeof(icmp_echo_packet_t)) {
		debug_icmp("     length too short!\n");
		return;
	}

	icmp_echo_packet_t *packet = (icmp_echo_packet_t *)&icmp_packet->data;

	const size_t data_length = icmp_packet_data_size - sizeof(icmp_echo_packet_t);
	const size_t size = sizeof(icmp_packet_t) + sizeof(icmp_echo_packet_t) + data_length;

	icmp_packet_t *icmp_reply = kcalloc(1, size);
	if (icmp_reply == NULL) { return; }
	...
	echo_reply->identifier = packet->identifier;
	echo_reply->sequence = packet->sequence;

	debug_icmp("      %s: copying %u bytes of echo data\n", __func__, (unsigned int)data_length);
	if (data_length != 0) {
		memcpy(echo_reply->data, packet->data, data_length);
	}
	icmp_reply->checksum = htons(net_calc_checksum((uint8_t *)icmp_reply, size));

	bool success = net_send_ipv4(netif, ethernet_packet->srcmac, ipv4_packet->identification, IPV4_TYPE_ICMP, netif->ip, ipv4_packet->srcip, (uint8_t *)icmp_reply, size);
	(void)success;
	kfree(icmp_reply);
}

static void net_handle_icmp(netif_t *netif, const ethernet_packet_t *ethernet_packet, size_t ethernet_length, ipv4_packet_t *ipv4_packet, size_t ipv4_data_length) {
	...
	const size_t icmp_packet_data_size = ipv4_data_length - sizeof(icmp_packet_t);
	...
	debug_icmp("    data_len: 0x%4x\n", (unsigned int)icmp_packet_data_size);
	...
	switch(icmp_packet->type) {
		case ICMP_TYPE_ECHO_REQUEST:
			...
			net_handle_icmp_echo(netif, ethernet_packet, ipv4_packet, icmp_packet, icmp_packet_data_size);
			break;
...
static void net_handle_ipv4(netif_t *netif, const ethernet_packet_t *ethernet_packet, size_t length) {
	...
	debug_ipv4("  length          : 0x%4x\n", (unsigned int)ipv4_packet_length);
	...
	if (!memcmp(ipv4_packet->dstip, netif->ip, 4)) {
		...
		switch(ipv4_packet->protocol) {
			case IPV4_TYPE_ICMP:
				debug_ipv4("   icmp\n");
				net_handle_icmp(netif, ethernet_packet, length, ipv4_packet, data_length);
				break;
			case IPV4_TYPE_TCP:
				debug_ipv4("   tcp\n");
				net_handle_tcp(netif, ethernet_packet, length, ipv4_packet, data_length);
				break;
			case IPV4_TYPE_UDP:
				debug_ipv4("   udp\n");
				net_handle_udp(netif, ethernet_packet, length, ipv4_packet, data_length);
				break;
			default:
...
static void net_handle_ipv6(netif_t *netif, const ethernet_packet_t *ethernet_packet, size_t length) {
	(void)netif;
...
static void net_handle_arp(netif_t *netif, const ethernet_packet_t *ethernet_packet, size_t length) {
	if (length < (sizeof(ethernet_packet_t) + sizeof(ipv4_packet_t))) {
	...
	switch (ntohs(arp_packet->opcode)) {
		case ARP_OPCODE_REQUEST:
			...
			if (!memcmp(netif->ip, arp_packet->dstpr, 4)) {
				...
				if (net_send_arp(netif, netif->mac, ethernet_packet->srcmac, ARP_OPCODE_REPLY,
...
static unsigned int ktask_net(const char *name, void *extra) {
	...
	while (1) {
		...
		debug_ethernet(" dst: %2x:%2x:%2x:%2x:%2x:%2x\n",
			packet->dstmac[0], packet->dstmac[1], packet->dstmac[2], packet->dstmac[3], packet->dstmac[4], packet->dstmac[5]);
		...
		switch (ntohs(packet->type)) {
			case ETHERNET_TYPE_ARP:
				debug_ethernet(" (arp)\n");
				net_handle_arp(netif, packet, length);
				break;
			case ETHERNET_TYPE_IPV4:
				debug_ethernet(" (ipv4)\n");
				net_handle_ipv4(netif, packet, length);
				break;
			case ETHERNET_TYPE_IPV6:
				debug_ethernet(" (ipv6)\n");
				net_handle_ipv6(netif, packet, length);
				break;
			default:
